...
dfCor <- as.data.frame(cor(df.scaled, SalePrice))
dfCor
...
```{r}
dfCor <- as.data.frame(cor(nonnumeric_data3, SalePrice))
dfCor
```
...
predictions <- predict(step4,newdata=dfTest_Final)
...
write_csv(predictions, "C:\\Users\\eric.hirsch\\Desktop\\people7.csv")
```

Removing outliers is not an improvement (.17020)  Neither is using the mean (.16984)  So we stick with step1, the original model. Transforming GrLivArea also hurts the model (.23417)  Best model is step 2.
